...
def connect(read_only=False):
    ...
    if not url and not kwargs["password"]:
        raise ValueError(
            "Set POSTGRES_PASSWORD in .env or DATABASE_URL in the environment."
        )
    try:
        ...
        raise RuntimeError(
            f"PostgreSQL operation failed (SQLSTATE {error.sqlstate or 'connection'})."
        ) from None
...
def load_database(data):
    ...
    with connect() as connection:
        ...
        connection.execute((ROOT / "sql/postgres/changes.sql").read_text())
        connection.execute(
            "SELECT revision FROM catalog_state WHERE singleton=1 FOR UPDATE"
        )
        connection.execute(
            "CREATE TEMP TABLE batch_products (LIKE products) ON COMMIT DROP"
        )
        with connection.cursor() as cursor:
            ...
            cursor.executemany(
                f"INSERT INTO products ({columns}) VALUES ({placeholders}) ON CONFLICT(product_id) DO UPDATE SET {updates}",
                records,
            )
        matches = " AND ".join(
            f"p.{c} IS NOT DISTINCT FROM b.{c}" for c in DATABASE_COLUMNS
        )
        count = connection.execute(
            f"SELECT COUNT(*) FROM batch_products b JOIN products p ON p.product_id=b.product_id WHERE {matches}"
        ).fetchone()[0]
