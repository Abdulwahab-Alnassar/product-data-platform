    ...
    if backend == "postgres":
        ...
        with connect(read_only=True) as connection:
            cursor = connection.execute(
                "SELECT * FROM product_analytics ORDER BY product_id"
            )
            data = pd.DataFrame(
        ...
        with closing(
            ...
            data = pd.read_sql_query(
                "SELECT * FROM product_analytics ORDER BY product_id", connection
            )
    else:
...
def filter_products(
    data, search="", categories=None, min_rating=0, include_unrated=True
):
    mask = data.product_name.str.contains(
