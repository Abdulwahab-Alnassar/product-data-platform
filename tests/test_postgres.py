...
def batch(monkeypatch):
    ...
    monkeypatch.setenv('DATABASE_URL', psycopg.conninfo.make_conninfo(url, options=f'-c search_path={schema}'))
    try:
        yield prepare_data(clean_data(load_data(ROOT / 'data/sample/amazon_sample.csv'))).head(3)
    finally:
        with psycopg.connect(url, autocommit=True) as con:
...
def test_upsert_and_null(batch):
    ...
    assert result.loc[changed.iloc[0].product_id, 'rating'] != result.loc[changed.iloc[0].product_id, 'rating']
...
def test_postgres_pipeline(batch, tmp_path):
    result = run_pipeline(ROOT / 'data/sample/amazon_sample.csv', tmp_path, backend='postgres')
